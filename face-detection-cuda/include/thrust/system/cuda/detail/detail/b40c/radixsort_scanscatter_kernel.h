...
/******************************************************************************
 * Appropriate substitutes to use for out-of-bounds key (and value) mOffsets
 ******************************************************************************/
	...
	int copy_section)
{
	...
	if (threadIdx.x < RAKING_THREADS_PER_PASS) {
		...
		PrefixScanOverLanes<SCAN_LANES_PER_PASS, LOG_RAKING_THREADS_PER_LANE, RAKING_THREADS_PER_LANE, PARTIALS_PER_SEG>(		// first pass is mOffset right by one
			raking_partial,
			warpscan, 
			copy_section);
	}
	...
	int extra[1])				
{
	...
	// Calculate scatter mOffsets (re-decode digits from keys: it's less work than making a second exchange of digits)
	#pragma unroll 
	for (int PASS = 0; PASS < (int) PASSES_PER_CYCLE; PASS++) {
	...
	int	*raking_partial)		
{
	...
	typename VecType<K, 2>::Type 	keypairs[PASSES_PER_CYCLE][SETS_PER_PASS];
	int2 							digits[PASSES_PER_CYCLE][SETS_PER_PASS];
	int2 							flag_offsets[PASSES_PER_CYCLE][SETS_PER_PASS];		// a byte mOffset
	int2 							ranks[PASSES_PER_CYCLE][SETS_PER_PASS];
	...
	if (threadIdx.x < RADIX_DIGITS) {
		...
		#pragma unroll
		for (int PASS = 0; PASS < (int) PASSES_PER_CYCLE; PASS++) {
			RecoverDigitCounts<SCAN_LANES_PER_PASS, RAKING_THREADS_PER_LANE, SETS_PER_PASS, SCAN_LANES_PER_SET>(		// first pass, mOffset by 1
				warpscan, 
				counts[PASS],
				PASSES_PER_CYCLE - PASS - 1);		// lower passes get copied right
		}
	...
	CtaDecomposition work_decomposition)
{
	...
	if (threadIdx.x < RAKING_THREADS_PER_PASS) {
		...
		if (threadIdx.x < RADIX_DIGITS) {
			...
			// needs someone else's because he always writes the zero mOffset.
