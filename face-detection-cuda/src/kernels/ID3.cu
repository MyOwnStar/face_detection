...
__global__
void ID3kernel(float *intImage, size_t stride, int *offsets, int windowSize, int numSubWindows, int scale,
               int *faceDetected, float *results)
{
   ...
   if (threadNum < numSubWindows)
   {
      ...
      for (int i = startX; (i + ID3_BASE_WIDTH * scale) < (startX + windowSize); i = i + ID3_SKIP_AMOUNT)
      { //use ID3_SKIP_AMOUNT * scale for it to scale up as identifier scales
         for (int j = startY; (j + ID3_BASE_HEIGHT * scale) < (startY + windowSize); j = j + ID3_SKIP_AMOUNT)
         {
            ...
            float midLeftBot = intImage[(i + ID3_BASE_WIDTH * scale / 2 - ID3_MID_WIDTH * scale / 2) * stride + j +
                                        ID3_BASE_HEIGHT * scale];
            float midRightBot = intImage[(i + ID3_BASE_WIDTH * scale / 2 + ID3_MID_WIDTH * scale / 2) * stride + j +
                                         ID3_BASE_HEIGHT * scale];
