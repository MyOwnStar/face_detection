...
__global__
void ID4kernel(float *intImage, size_t stride, int *offsets, int windowSize, int numSubWindows, int scale,
               int *faceDetected, float *results)
{
   ...
   if (threadNum < numSubWindows)
   {
      ...
      for (int i = startX; (i + ID4_BASE_WIDTH * scale) < (startX + windowSize); i = i + ID4_SKIP_AMOUNT)
      { //use ID4_SKIP_AMOUNT * scale for it to scale up as identifier scales
         for (int j = startY; (j + ID4_BASE_HEIGHT * scale) < (startY + windowSize); j = j + ID4_SKIP_AMOUNT)
         {            // take important corners from image
            ...
            float midLeftBot = intImage[(i + ID4_BASE_WIDTH * scale / 2 - ID4_MID_WIDTH * scale / 2) * stride + j +
                                        ID4_BASE_HEIGHT * scale];
            float midRightBot = intImage[(i + ID4_BASE_WIDTH * scale / 2 + ID4_MID_WIDTH * scale / 2) * stride + j +
                                         ID4_BASE_HEIGHT * scale];
            ...
            float fitValue =
                    (-midRightBot - midLeftTop + midRightTop + midLeftBot) * 2 + lowerRight + upperLeft - upperRight -
                    lowerLeft;
